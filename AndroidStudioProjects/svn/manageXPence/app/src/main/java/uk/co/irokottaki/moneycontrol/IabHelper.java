...
public class IabHelper {
        ...
        if (resultCode == Activity.RESULT_OK && responseCode == BILLING_RESPONSE_RESULT_OK) {
            ...
            try {
                ...
                if (!Security.verifyPurchase(mSignatureBase64, purchaseData, dataSignature)) {
                    ...
                    result = new IabResult(IABHELPER_VERIFICATION_FAILED, "Signature verification" +
                            " failed for sku " + sku);
                    if (mPurchaseListener != null)
    ...
    private Inventory queryInventory(boolean querySkuDetails, List<String> moreSkus) throws
            IabException {
     ...
     *
     * @param querySkuDetails if true, SKU details (priceOfPRO, description, etc) will be queried
     *                        as well
    ...
    private Inventory queryInventory(boolean querySkuDetails, List<String> moreItemSkus,
        ...
        try {
            ...
            throw new IabException(IABHELPER_REMOTE_EXCEPTION, "Remote exception while refreshing" +
                    " inventory.", e);
        } catch (JSONException e) {
     ...
     */
    private void queryInventoryAsync(final boolean querySkuDetails, final List<String> moreSkus,
    ...
    public void queryInventoryAsync(boolean querySkuDetails, QueryInventoryFinishedListener
            listener) {
    ...
    private int queryPurchases(Inventory inv, String itemType) throws JSONException,
            RemoteException {
        // Query purchases
        ...
        (new Thread(new Runnable() {
            public void run() {
                ...
                for (Purchase purchase : purchases) {
                    try {
                        consume(purchase);
                        results.add(new IabResult(BILLING_RESPONSE_RESULT_OK, "Successful consume" +
                                " of sku " + purchase.getSku()));
                    } catch (IabException ex) {
